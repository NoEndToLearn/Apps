...
namespace Apps.Web.Areas.MIS.Controllers
{
    public class ManageArticleController : BaseController
    {
        ...
        {
            List<MIS_ArticleModel> list = articleBll.GeAuditedList(ref pager, true, queryStr);
            var json = new
            {
                total = pager.total,
                rows = (from r in list
        ...
        public JsonResult Audit(string id)
        {
            ...
            {
                if (articleBll.Audit(ref errors, id, GetCurrentUserId()))
                {
                    LogHandler.WriteServiceLog(GetCurrentUserId(), "Id:" + id, "成功", "审核", "IMIS_ArticleRepository");
                    return Json(JsonHandler.CreateMessage(1, Suggestion.AuditSeccueed));
                }
                else
                {
                    string ErrorCol = errors.Error;
