...
function addTab(subtitle, url, name, icon) {
    var exists = false;
    console.log($("#mainTab ul li a").text);
    $("#mainTab ul li a").each(function (i, e) {
        if (e.text == subtitle) {
            exists = true;
        }
    });
    if (!exists) {
        $("#mainTab ul .active").removeClass("active");
        $("#mainTab .tab-content .active").removeClass("active");
        $("#mainTab ul").append("<li class='active'><a href='#" + name + "' data-toggle='tab' aria-expanded='false'>" + subtitle + "</a></li>");
        $("#mainTab .tab-content").append("<div class='tab-pane active' id='" + name + "'>" + createFrame(url) + "</div>");
    } else {
        $("#mainTab ul .active").removeClass("active");
        $("#mainTab .tab-content .active").removeClass("active");
        //$("#mainTab").tabs('select', subtitle);
        $("#mainTab ul li a").each(function (i, e) {
            if (e.text == subtitle) {
                e.addClass("active"); // 追加样式
            }
        });
        $("#mainTab div").each(function (i, e) {
            if (e.text == subtitle) {
                e.addClass("active"); // 追加样式
            }
        });
