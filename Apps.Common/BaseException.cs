namespace Apps.Common
{
    ...
    public class BaseException
    {
        ...
        private bool isShow;
        private readonly Exception outermostException = null;
        ...
        {
            string stackInfomation = this.StackInfo;
            string[] strArray;
            if (stackInfomation == null)
            {
                return stackInfomation;
            }

            strArray = stackInfomation.Split(new string[] { "位置", "行号" }, StringSplitOptions.RemoveEmptyEntries);
            if (strArray.Length >= 3)
            {
                stackInfomation = strArray[1];
                if (stackInfomation.LastIndexOf(":") == (stackInfomation.Length - 1))
                {
                    stackInfomation = stackInfomation.Substring(0, stackInfomation.Length - 1);
                }
        ...
        private string GetSourceErrorRowID()
        {
            ...
            strArray = stackInfomation.Split(new string[] { "行号" }, StringSplitOptions.RemoveEmptyEntries);
            if (strArray.Length >= 2)
            {
                stackInfomation = strArray[1].Trim();
                string[] strArray2 = stackInfomation.Split(new string[] { "\r\n" }, StringSplitOptions.RemoveEmptyEntries);
                if (strArray2.Length >= 2)
                {
                    stackInfomation = strArray2[0];
                }
        ...
        private string GetTargetSite(Exception ex)
        {
            ...
            if (ex != null)
            {
                ...
                MethodBase tarSite = ex.TargetSite;
                if (tarSite != null)
                {
                    str = string.Format("{0}.{1}", tarSite.DeclaringType, tarSite.Name);
                }
            }
